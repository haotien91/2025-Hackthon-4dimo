...
## 5. 若使用 AI，需額外補充的項目（AI Usage Declaration）
...
## 6. 注意事項 / Important Notices

> [!IMPORTANT]
> 部署或開發時請特別注意以下事項：

### 1. CORS 跨域設定
後端 API 預設啟用 CORS（`ENABLE_CORS=true`）。若前端與後端不在同一網域，請確保：
- 後端的 `ENABLE_CORS` 設為 `true`
- 若有特定網域限制需求，請修改 `app/main.py` 中的 CORS 設定

### 2. HTTPS / HTTP 混合內容問題
若前端部署於 HTTPS 環境（如 Vercel），則：
- 後端 API **必須**也使用 HTTPS，否則瀏覽器會阻擋 HTTP 請求（Mixed Content）
- 圖片 URL 同樣需要 HTTPS，請確保 `API_BASE_URL` 設定正確
- 本地開發可使用 HTTP，但生產環境建議全面使用 HTTPS
...
| 變數 | 說明 | 需同步的專案 |
|------|------|-------------|
| `API_BASE_URL` | 後端公開 URL | event_api, art-pass |
| `ART_PACE_URL` | 前端公開 URL | TownPass |

### 4. 資料更新機制
- `events.json` 來自台北市文化快遞，需定期執行爬蟲更新
- 爬蟲腳本位於 `event_api/crawler/`（若有）
- 建議使用 Cron Job 或 GitHub Actions 定期排程
